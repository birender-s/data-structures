...
//                lookup.put(key, LCS_String(X, Y, m - 1, n - 1, lookup) + X.charAt(m - 1));
            arrayList = appendToListItems(LCS_String(X, Y, m - 1, n - 1, lookup), X.charAt(m - 1));
            insertToMap(key, arrayList, lookup);
        }
        else {
...
//                    lookup.put(key, secondLcs);
    ...
    private static ArrayList<String>  appendToListItems(ArrayList<String> list, char chr){
        ...
        if (list != null && list.size() > 0)
            list.stream().forEach(v -> {listTmp.add(v + chr);});
        else {
            listTmp.add("" + chr);
        }
